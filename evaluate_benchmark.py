...
def main(args):
    ...
    for i in range(len(testset)):
        depth_pred = model(paddle.unsqueeze(testset[i]['image'], axis=0))
        depth_pred = DepthNorm(depth_pred)[0][0]
        depth_pred_flip = model(paddle.unsqueeze(testset[i]['image'][:, :, ::-1], axis=0))
        depth_pred_flip = DepthNorm(depth_pred_flip)[0][0]
        depth_gt = testset[i]['depth'][0]
        # Scaled
        scaled = paddle.mean(depth_gt) / paddle.mean(depth_pred)
        preds[i] = (depth_pred * 0.5 + paddle.flip(depth_pred_flip, axis=-1) * 0.5) * scaled
        gts[i] = depth_gt
